...
Whether it then runs inline under test depends on the action, not only on phpunit.xml. QUEUE_CONNECTION=sync makes inline the default, but an action naming its own connection - SummariseVideo declares `public ?string $connection = 'summaries'` for its retry_after - overrides it, so dispatching that one under test queues it rather than running it. Call execute() directly, or fake the queue and assert what was pushed. The summariseVideo() helper in tests/Helpers/Functions.php does the first: it claims the row the way SummariseVideo would and then runs the five steps in order, so a test gets a whole attempt without a queue.
...
So the guard against two attempts is the claim, not the lock. The entry action claims with a conditional update and hands every step the row id and a claim token; each step reads and writes conditional on it. A step given only the id could not tell an attempt that was written off and replaced from the one it belongs to.

The claim is a token, not a timestamp, and that is load-bearing. started_at is a timestamp(0) and Laravel's query grammar binds dates as 'Y-m-d H:i:s' on both the write and the comparison, so two attempts on one row inside the same wall-clock second were indistinguishable. Raising the column's precision does not help - the truncation is in the grammar. started_at keeps the job it is good at: what the page counts up from. Whatever clears it for a retry must clear the token too, or the old attempt's steps still match.

Every write is conditional, not just the ones in execute(). failed() and giveUp() run minutes after the claim was checked - a model call or a yt-dlp run is most of that - and by then the attempt may have been written off and replaced. Writing unconditionally there marks the live attempt failed, and its remaining steps then find the row no longer pending and quietly do nothing: a summary nobody gets, already paid for.
...
Guard anything paid against redelivery, not just against a retry. A worker can finish a step and be killed before it deletes the job, and retry_after hands the same job to the next worker; nothing about the row says so, because only the last step moves it off pending. Each step that costs money returns early when what it produces is already there. Test the guard on the value, not on null - an empty string stored where a model answer should be satisfies a null check for good, and no retry can recover it.
